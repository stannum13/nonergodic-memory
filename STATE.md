...
Register before running: train matched length-eight Transformer checkpoints at overlap 0.00/.35 and score them on the same eight-token windows and full-history Bayesian targets as the length-64 original-index restarts. Hold architecture width/depth, seeds, probe-fit/test batches, and window tokens fixed; vary only the training sequence length. Prediction: a short-trained model improves eight-token predictive KL relative to the length-64 restarted model if part of its penalty is window-input distribution shift; it cannot recover older-token Bayes information. This is a diagnostic, not an already obtained result.
