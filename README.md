...
The separate Mess3 experiment reproduces the paper's exact two-component observation process and probes the six weighted joint beliefs `p(component,state | context)`. Its initial fixed-pool CPU run failed the registered trained-over-untrained prediction. A subsequent registered diagnosis found why this result was not yet a strong geometry test: the model had learned less predictive structure than an eight-token exact Bayesian baseline. Fresh sequences improve predictive generalization by the original 768-update budget, but component recovery remains near zero there. Only after 3,072 fresh-data updates does the model recover 80–84% of the uniform-to-Bayes predictive gap and reach deeper-layer joint-belief R² of 0.62–0.73; fixed-pool training instead overfits sharply. The geometry recovery therefore reflects fresh data plus a fourfold larger budget, not diversity alone. See the [fidelity reproduction and training-diversity diagnosis](report.md#direct-mess3-fidelity-reproduction).
