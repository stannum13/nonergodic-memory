...
The separate Mess3 experiment reproduces the paper's exact two-component observation process and probes the six weighted joint beliefs `p(component,state | context)`. Its smaller CPU Transformer fails the registered trained-over-untrained prediction in every seed (mean joint R² 0.321 trained versus 0.336 untrained). See the [fidelity table and seed-by-seed result](report.md#direct-mess3-fidelity-reproduction); the earlier two-state experiments remain a conceptual reproduction and causal extension.
